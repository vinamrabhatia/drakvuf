...
The project here can be distinctly divided into 2 parts here.

1. Modifying libinjector to execute arbitrary functions:
The earlier implementation of the libinjector hijacks an arbitrary process and checks if kernel32.dll is loaded. It sets up the stack for the execution of CreateProcessA function. The first part of the project was to extend the libinjector library so that it can call arbitrary functions inside the Windows kernel, along with the arguments which we provide. I came up with a couple of approaches to get the first part done, and after a good discussion with my mentor, we firmed up on a idea. I was earlier trying to trap the sys
...
Getting the GUID for 32-bit machines didn't work as it was working for 64 bit machines, and a significant chuck of time went away there. A quick glimpse of terminal commands which did the job.

```
$ sudo kpartx -l /dev/vgpool_32bit/win7_sp1_32  # This will show all the partitions in the volume, e,g
...
$ sudo mount -o ro /dev/mapper/vgpool_32bit/win7_sp1_32_2 /mnt  # might change partition to find the Windows/ folder
...
Then, I came across <b>LibFuzzer</b>. This gave me some hope since with this, one can kinda do targeted fuzzing of functions, also it is guided in-process fuzzing. (everything in a single process, so probably faster). But with my understanding, for this to work, I think the source code is a necessity, and we are trying to fuzz functions in a closed source running Kernel.
...
However, with further discussions, it came to my understanding that it will probably be a better idea to keep everything going within the hypervisor.

In other terms, the coverage guidance part of AFL is telling AFL which inputs to mutate more because a certain new path is discovered. Hence, 'the newly discovered path' is the trigger here. AFL gets this trigger with the instrumentation. However, we can also try to monitor for other types of behaviour, which can help us to mutate our inputs. Some of the possible cases can be reaching a particular kernel function (and this feedback can be obtained by setting up a trap at this particular function.

During this peak time of the project, I had to go for my undergraduate convocation at my university, BITS Pilani Goa. And later during the same week, I had my flights to the States, where I am starting Masters in Information Security at Carnegie Mellon University. Juggling with settling down in a new place, going through necessary orientations and preparing for the classes to be taken this current fall, I had a little time left for the final few days of GSoC.

I realised that I really need to figure out how the coverage guided thing actually gives the feedback and what all possible types of feedback can be given back to the AFL. I couldn't find anything on it available in the docs, so had to check out the code.

Struggling through that for a few days and with the end of the project coming nearer, I  thought it might be better that first I implement a simple fuzzer as a proof of concept of hooking syscalls, and restoring snapshots on crash, to which a more sophisticated fuzzer can be plugged into later. After multiple attempts, I was able to create clone VM machines running the kernel-injector script with the arguments that I supply.
...
src/fuzzer achieves the remaining project. The fuzzer takes in all the necessary arguments, clone a snapshot of the running guest, calls the functions and returns back. It is meant to take arguments of the target function from a file supplied by the fuzzer.
...
clone_script, kernel_injector_script and cleanup script are available in the tools directory.

So, a simple fuzzer as a proof of concept of hooking syscalls, and restoring snapshots on crash, has been implemented.

TODOs:

A more sophisticated fuzzer needs to be plugged into the existing framework.

The main leftover of the project is to make the fuzzer get the feedback from the kernel, and accordingly redirect/mutate the possible inputs. This can be done by figuring out 'interesting test cases' and supplying them into an existing fuzzer. Or we can also aim towards dynamic instrumentation of the kernel code through the hypervisor, giving up much more information to supply t the fuzzer. 
